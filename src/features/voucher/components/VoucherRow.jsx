...
const VoucherRow = ({voucher:{id,voucher_id,customer_name,customer_email,sale_date,tax,total}}) => {
    ...
    <tr className="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
       ...
       <span className='font-slate-600 text-xs'>{customer_email}</span>
              ...
              <td className="px-6 py-4 text-end">
                {total}              </td>
              <td className="px-6 py-4 text-end">
                <ShowDateTime timestamp={sale_date} />
              ...
              <HiOutlineTrash />
                ...
                <Link to={`/dashboard/voucher-details/${id}`} className="px-4 py-2 text-sm font-medium text-slate-600 bg-transparent border border-slate-600 rounded-e-lg hover:bg-slate-600 hover:text-white focus:z-10 focus:ring-2 focus:ring-gray-500 focus:bg-slate-600 focus:text-white dark:border-white dark:text-white dark:hover:text-white dark:hover:bg-gray-700 dark:focus:bg-gray-700"
                >
                  <HiArrowRight />
                </Link>
                </div>
